...
namespace vaterlangen\CardDavBundle\NativePHP;
...
/**
 * SOURCE: https://github.com/graviox/CardDAV-PHP | https://github.com/christian-putzke/CardDAV-PHP
 * 
 * 
 * 
 * CardDAV PHP
 ...
 * --------------------
 * $carddav = new CardDavClient('https://davical.example.com/user/contacts/');
...
class CardDavClient
{
	...
	*/
	...
	/**
	 * Forces the vCard to have valid UID field
	 *
	 * @param	string	$vcard		vCard
	 * @param	string	$vcard_id	unique ID
	 * @return	string				vCard
	 */
	private function force_uid($vcard, $vcard_id)
	{
		$vcard_ary = array();
		...
		foreach (explode("\n", str_replace("\r\n", "\n", $vcard)) as $line)
		{
			...
			{
				$vcard_ary[] = "UID:$vcard_id";
			}
				
			// remove existing UID fields
			if (!preg_match("/[a-zA-Z]+/", $line) || preg_match("/^UID:/", $line))
			{
				continue;
			}
	...
	public function add($vcard, $vcard_id = null)
	{
		...
		$vcard		= $this->force_uid($vcard, $vcard_id);
	...
	private function generate_vcard_id()
	{
		...
		try
		{
			$carddav = new CardDavClient($this->url);
